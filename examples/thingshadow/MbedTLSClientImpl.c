...
void const* MbedTLSClientImpl_socket_connect(SocketConnectionParameters_t *pConnParams, void *pUserData, void *___id)
{
  ...
  if (pConnParams->pSecureConnectionParams != NULL && pConnParams->pSecureConnectionParams->maxDataRate != TLS_MAX_DATA_RATE_NONE) 
  {
    if ((ret = mbedtls_ssl_conf_max_data_rate(&(pTLSDataParams->conf), pConnParams->pSecureConnectionParams->maxDataRate)) != 0) 
    {
      printf("Failed! mbedtls_ssl_conf_max_data_rate returned %i\n", ret);
      MbedTLSClientImpl_deleteMbedTLSNetwork(pNetwork, true, ___cid);
      return NULL;
    }
  }
